...
public:
    ...
    static BoundaryCondition ASE( double c_star ){
        return BoundaryCondition(5, c_star);
    }
    ...
    double value(double t) const{
        assert(type_==1 || type_==3 || type_==5); // only applys for dirichlet and fixed flux
...
private:
    ...
    BoundaryCondition( int type, double val ){
        assert( type==1 || type==3 || type==5 ); // ensure either dirichlet, prescribed flux or ASE
        type_ = type;
        value_ = val;
    }
...
public:
    ...
    double eta() const {return eta_;}; // fluid compressibility
    double g() const {return g_;};
    double rho_0() const {return rho_0_;};

    Constants( double mu, double eta, double g, double rho_0 ):
...
private:
    ...
    double eta_; // density coupling coefficient
    double g_; // gravity
