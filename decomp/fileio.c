...
{
    int i, j, k, ijk, n_nodes, n_elements, node, max_el, pos, nnz, nnz_max;
    double *node_x, *node_y, *node_z;
    int  *node_num_elements, *node_num_bcs, *node_list, **node_bcs; // UPDATED
    int **node_elements;
    ...
    {
        fprintf( stderr, "\tERROR : mesh_load() : unable to open file %s for input\n", fname );
        return 0;
    }
    ...
    fscanf( fid, "%d %d", &n_nodes, &n_elements );
    ...
    node_bcs = (int**)malloc( n_nodes*sizeof(int*) );
    node_num_bcs = (int*)malloc( n_nodes*sizeof(int) );
    node_num_elements = (int*)calloc( n_nodes, sizeof(int) );

    // read in coordinates of nodes
    for( i=0; i<n_nodes; i++ )
    {
        //fscanf( fid, "%lf %lf %lf %d", node_x+i, node_y+i, node_z+i, node_bc+i  );
        fscanf( fid, "%lf %lf %lf %d", node_x+i, node_y+i, node_z+i, node_num_bcs+i );
        node_bcs[i] = NULL;
        if( node_num_bcs[i] ){
            node_bcs[i] = (int*)malloc( node_num_bcs[i]*sizeof(int) );
            for( j=0; j<node_num_bcs[i]; j++ ){
                fscanf( fid, "%d", node_bcs[i]+j );
            }
        }
    }
    ...
    for( i=0; i<n_elements; i++ )
    {
        ...
        /*
        fscanf( fid, "%d %d %d", &el->n_nodes, &el->n_edges, &el->n_faces );
        */
        fscanf( fid, "%d %d ", &el->type, &el->physical_tag );

        if( el->type==ELEMENT_TRIANGLE )
        {
            el->n_nodes = 3;
            el->n_faces = 3;
        } 
        if( el->type==ELEMENT_QUAD )
        {
            el->n_nodes = 4;
            el->n_faces = 4;
        } 
        if( el->type==ELEMENT_TETRAHEDRON )
        {
            el->n_nodes = 4;
            el->n_faces = 4;
        } 
        else if (el->type==ELEMENT_HEXAHEDRON )
        {
            el->n_nodes = 8;
            el->n_faces = 6;
        }
        else if (el->type==ELEMENT_PRISM )
        {
            el->n_nodes = 6;
            el->n_faces = 5;
        }
        ...
        for( j=0; j<el->n_nodes; j++ )
        {
            ...
            {
                fprintf( stderr, "ERROR : mesh_read() : invalid element->node data (%d in %d)\n", el->nodes[j], n_nodes );
                return 0;
            }
        }
    ...
    mesh->node_y   = node_y;
    mesh->node_z   = node_z;
    mesh->node_bcs  = node_bcs;
    mesh->node_num_bcs  = node_num_bcs;
...
/*
    WARNING TODO:
    has been broken by new mesh definition. Compiles, but needs to be
    fixed if it is to be used
*/
int mesh_load_binary( Tmesh_ptr mesh, char *fname )
{
    ...
    fprintf(stderr, "\tERROR : mesh_load_binary() is out of date" );
    error(1);
...
int mesh_compiled_save( Tmesh_ptr mesh, char *fname )
{
    ...
    fwrite( mesh->node_num_bcs, sizeof(int), n_nodes, fid );
    fwrite( mesh->node_num_elements, sizeof(int), n_nodes, fid );

    for( i=0; i<n_nodes; i++ )
        fwrite( mesh->node_bcs[i], sizeof(int), mesh->node_num_bcs[i], fid );
    for( i=0; i<n_nodes; i++ )
    ...
    for( i=0; i<n_elements; i++ )
    {
        fwrite( &mesh->elements[i].type, sizeof(int), 1, fid );
        fwrite( &mesh->elements[i].physical_tag, sizeof(int), 1, fid );
...
int mesh_compiled_read( Tmesh_ptr mesh, char *fname )
{
    ...
    mesh->node_num_bcs = (int*)malloc( sizeof(int)*n_nodes );
    ...
    fread( mesh->node_num_bcs, sizeof(int), n_nodes, fid );
    fread( mesh->node_num_elements, sizeof(int), n_nodes, fid );
    mesh->node_bcs = (int**)malloc( sizeof(int*)*n_nodes );
    mesh->node_elements = (int**)malloc( sizeof(int*)*n_nodes );
    for( i=0; i<n_nodes; i++ ){
        if( mesh->node_num_bcs[i] ){
            mesh->node_bcs[i] = malloc( sizeof(int)*mesh->node_num_bcs[i] );
            fread( mesh->node_bcs[i], sizeof(int), mesh->node_num_bcs[i], fid );
        }
        else
            mesh->node_bcs[i] = NULL;
    }
    for( i=0; i<n_nodes; i++ ){
    ...
    for( i=0; i<n_elements; i++ )
    {
        fread( &mesh->elements[i].type, sizeof(int), 1, fid );
        fread( &mesh->elements[i].physical_tag, sizeof(int), 1, fid );
        fread( &mesh->elements[i].tag,   sizeof(int), 1, fid );

        if( mesh->elements[i].type==ELEMENT_TRIANGLE )
        {
            mesh->elements[i].n_nodes = 3;
            mesh->elements[i].n_faces = 3;
        } 
        else if( mesh->elements[i].type==ELEMENT_QUAD )
        {
            mesh->elements[i].n_nodes = 4;
            mesh->elements[i].n_faces = 4;
        } 
        else if( mesh->elements[i].type==ELEMENT_TETRAHEDRON)
        {
            mesh->elements[i].n_nodes = 4;
            mesh->elements[i].n_faces = 4;
        }
        else if( mesh->elements[i].type==ELEMENT_HEXAHEDRON)
        {
            mesh->elements[i].n_nodes = 8;
            mesh->elements[i].n_faces = 6;
        }
        else if( mesh->elements[i].type==ELEMENT_PRISM)
        {
            mesh->elements[i].n_nodes = 6;
            mesh->elements[i].n_faces = 5;
        }
        else
        {
            printf( "ERROR : invalid element type in input file\n" );
            exit(0);
        }
...
int mtx_CRS_dist_output( Tmtx_CRS_dist_ptr A, char *stub )
{
    ...
    success = mtx_CRS_output_matlab( &A->mtx, fname );
...
int domain_print( FILE *fid, Tdomain_ptr dom )
{
    int i, n_dom=dom->n_dom;
...
int domain_load( Tdomain_ptr dom, char *stub, TMPI_dat_ptr This )
{
    ...
    {
        if( !This->this_proc )
            printf( "ERROR : domain_load() : unable to open file %s\n", fname );
        return 0;
    }
...
// read a line of text from a stream. returns after the first line of
// text that doesn't begin with a comment character #
// line must be allocated with MAX_INPUT_LINE_LENGTH
char *fgetline( FILE *fid, char *line )
{
    char *s, first_char[256];
    int count;
    
    do
    {
        s = fgets( line, MAX_INPUT_LINE_LENGTH, fid );
        count = sscanf( s, "%s", first_char );
    }
    while( s && (first_char[0]=='#' || !count) );
    
    return s;
}

int fgetvar( FILE *fid, char *format_string, void *var )
{
    char line[MAX_INPUT_LINE_LENGTH]; 
    
    if( !fgetline( fid, line ) )
    {
        fclose( fid );
        return 0;
    }
    
    return sscanf( line, format_string, var );
}
