...
//#pragma omp parallel for schedule(static) shared(rho_face_ptr, ja, ia) private(e, rho_edge)
    ...
    void VarSatPhysicsImpl<CoordHost,CoordDevice>::process_fluxes( double t, const mesh::Mesh &m )
    {
        ...
        // There is an issue here where minlin chooses an inefficient method when a range
        // is specified in qdotn_faces, however works fine when just qdotn_faces is passed.
        // This needs to be fixed, because the efficient option is not stricly correct and
        // does not work with LIN_DEBUG set.
        //std::cerr << "===============================================================" << std::endl;
        //std::cerr << "using inefficient option abcABC" << std::endl;
        ...
        // determine the flux over the dirichlet faces
        // this serves no purpose in this model, but is useful
        // for more advanced simulations that involve advection
        // of contaminants
        //TVecDevice flux_tmp(dirichlet_nodes_.dim());
        // flux_tmp has the total flux, in or out
        //dirichlet_matrix.matvec(qdotn_faces, flux_tmp);
        //qdotn_faces.at(dirichlet_faces);
        /* =================================================================================
            ...
            if( is_dirichlet_h_vec_[cvf.back().id()] )
            {
                ...
                for (int ii = 0; ii < v.scvs(); ++ii) {
                    ...
                    for (int j = 0; j < scv.cvfaces(); ++j) {
                        ...
                        if( subcvf.id()<m.interior_cvfaces() ||
                            (  !boundary_condition_h(subcvf.boundary()).is_dirichlet() &&
                                boundary_condition_h(subcvf.boundary()).type()!=7 &&
                                boundary_condition_h(subcvf.boundary()).type()!=8)
                            )
                        {
