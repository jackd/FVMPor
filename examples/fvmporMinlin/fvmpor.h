...
protected:
    ...
    TVecDevice M_flux_faces; // mass flux at CV faces // DEVICE - need both for easy access in the FVM_Assembler
    TVec       M_flux_faces_host; // mass flux at CV faces // HOST  - need both for easy access in the FVM_Assembler
...
// **************************************************************************
	...
	std::cout << "with faces internal " << m.interior_cvfaces() << std::endl;
        ...
        std::cerr << "Time to initialise : " << timer.toc() << " seconds" << std::endl;
    }
    ...
    void VarSatPhysicsImpl<CoordHost,CoordDevice>::process_faces_lim( const mesh::Mesh &m )
    {
        ...
        if(CoordTraits_<CoordDeviceInt>::is_device()){
            lin::gpu::collect_edges(
                          ...
                          edge_weight_front_.data(), edge_weight_front_.data(),
    ...
    void VarSatPhysicsImpl<CoordHost,CoordDevice>::process_fluxes( double t, const mesh::Mesh &m )
    {
        ...
        //cudaMemcpy(qdotn_faces.data()+m.interior_cvfaces(), qdotn_faces_bnd.data(), faces_bnd*sizeof(double), cudaMemcpyHostToDevice);
        ...
        */
        ...
        ======================================================================== */
        M_flux_faces.at(m.interior_cvfaces(), lin::end) = constants().rho_0()*qdotn_faces.at(m.interior_cvfaces(), lin::end);
    }
    ...
    void VarSatPhysicsImpl<CoordHost,CoordDevice>::process_volumes_psk( const mesh::Mesh &m )
    {
        ...
        for( std::map<int, int>::iterator it=zones_map_.begin(); it!=zones_map_.end(); it++){
            ...
            /*
            if(CoordTraits_<CoordDeviceInt>::is_device()){
                lin::gpu::assign_indirect_mullvv( q_front_[zone].dim(), krw_scv[zone].data(), edge_weight_front_.data(), 1, krw_faces_lim.data(), 1,
                                     q_front_[zone].data(), n_front_[zone].data(), p_front_[zone].data());
                lin::gpu::add_indirect_mullvv( q_back_[zone].dim(), krw_scv[zone].data(), edge_weight_back_.data(), 1, krw_faces_lim.data(), 1,
                                     q_back_[zone].data(), n_back_[zone].data(), p_back_[zone].data());
            }
            */
        }
