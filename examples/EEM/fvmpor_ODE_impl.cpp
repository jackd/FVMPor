...
namespace fvmpor {
                                       ...
                                       TVecDevice &res)
    {
        ...
        // subtract the lhs
        // EEMCHANGE - return the derivative
        //res -= mul(hp_vec_,ahh_vec);
        res /= ahh_vec;

        // Dirichlet boundary conditions
        // EEMCHANGE - to support dirichlet BCs we will need to change how they
        // are expressed. Currently we insist that there are no Dirichlet BCs
        assert(dirichlet_nodes_.size()==0);
